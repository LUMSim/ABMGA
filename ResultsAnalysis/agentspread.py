# Purpose: To read through the results of the analysis of GS/GAS runs and put the min/max data into a useable format to copy/paste 
# into the CSV files. Need to average the three runs of the same parameters and find standard deviation of them.
# IDENTICAL TO MINMAXGAS.PY EXCEPT THE CALCULATIONS IN DEAL_WITH_LINE FUNCTIONS
...
def deal_with_line(split, values):
	...
	if (split[0][0]=="f" and (len(split) == 5 or len(split) == 6)) or (split[0][0]=="t" and (len(split) == 7 or len(split) == 8)) : #valid data row
		...
		wmin = int(split[3])
		wmax = int(split[4])
		values[0].append((smax-smin))
		values[1].append((wmax-wmin))
		if split[0][0]=="t":
			values[2].append(float(split[6])-float(split[5]))
	else:
...
def allrows():
	try:
		...
		while line != "":
			values = [[],[],[]] #one for each agent type
